...
class BirdeyeRender(object):
  def __init__(self, world, params):
    ...
    self.boundary_surface = pygame.Surface((self.map_image.surface.get_width(),self.map_image.surface.get_height()))
    self.boundary_surface.set_colorkey(COLOR_BLACK)

    self.lane_boundary_surface = pygame.Surface((self.map_image.surface.get_width(),self.map_image.surface.get_height()))
    self.lane_boundary_surface.set_colorkey(COLOR_BLACK)
  ...
  def render_boundary(self,surface,boundary,world_to_pixel,color):
    ...
    # test for draw ref point
    # point = self.ref_point
    # self.render_waypoint(surface,point,world_to_pixel)

    # draw lane boundary
    # self.render_lane_boundary(surface,self.ref_point,world_to_pixel)
  ...
  def render_waypoint(self,surface,point,world_to_pixel):
    color = pygame.Color(0,0,255)
    waypoint = world_to_pixel(carla.Location(x=point[0],y=point[1]))
    radius = 1
    pygame.draw.circle(surface,color,waypoint,radius,0)
  

  def render_lane_boundary(self,surface,ref_point,world_to_pixel):
    color = pygame.Color(0,255,0)
    lane_change = ref_point.lane_change

    def draw_left(p):
      if p is None: return
      print("left_ref_point: {}".format(p))
      prev_wps = p.previous_until_lane_start(0.2)
      print("Previous wp size: {}.".format(len(prev_wps)))
      next_wps = p.next_until_lane_end(0.2)
      print("Next wp size: {}.".format(len(next_wps)))
      lane_wps = prev_wps + next_wps
      # print("Previous wp size: {}, next wp size: {}".format(len(prev_wps),len(next_wps)))
      if len(lane_wps) <= 1: return
      left_side = [lateral_shift(w.transform, -w.lane_width*0.5) for w in lane_wps]
      left_lane_boundary = []
      for loc in left_side:
        if (loc.x-self.ego_loc.x)**2 + (loc.y-self.ego_loc.y)**2 < 144:
          left_lane_boundary.append(carla.Location(x=loc.x,y=loc.y))
      if len(left_lane_boundary) > 0:
        print(left_lane_boundary)
        left_lane_boundary = [world_to_pixel(p) for p in left_lane_boundary]
        pygame.draw.lines(surface,color,False,left_lane_boundary,1)

    def draw_right(p):
      if p is None: return
      print("right_ref_point: {}".format(p))
      prev_wps = p.previous_until_lane_start(0.2)
      print("Previous wp size: {}.".format(len(prev_wps)))
      next_wps = p.next_until_lane_end(0.2)
      print("Next wp size: {}.".format(len(next_wps)))
      ...
      # print("Previous wp size: {}, next wp size: {}".format(len(prev_wps),len(next_wps)))
      if len(lane_wps) <= 1: return
      right_side = [lateral_shift(w.transform, w.lane_width*0.5) for w in lane_wps]
      right_lane_boundary = []
      for loc in right_side:
        if (loc.x-self.ego_loc.x)**2 + (loc.y-self.ego_loc.y)**2 < 144:
          right_lane_boundary.append(carla.Location(x=loc.x,y=loc.y))
      if len(right_lane_boundary) > 0:
        right_lane_boundary = [world_to_pixel(p) for p in right_lane_boundary]
        pygame.draw.lines(surface,color,False,right_lane_boundary,1)
    ...
    if lane_change == carla.LaneChange.Both:
      print(ref_point.get_left_lane(),ref_point.get_right_lane())
      draw_left(ref_point.get_left_lane())
      draw_right(ref_point.get_right_lane())
    elif lane_change == carla.LaneChange.Left:
      print(ref_point.get_left_lane())
      draw_left(ref_point.get_left_lane())
      draw_right(ref_point)
    elif lane_change == carla.LaneChange.Right:
      print(ref_point.get_right_lane())
      draw_left(ref_point)
      draw_right(ref_point.get_right_lane())
    else:
      draw_left(ref_point)
  ...
  def render(self, display, render_types=None):
    ...
    self.boundary_surface.fill(COLOR_BLACK)
    ...
    self.lane_boundary_surface.fill(COLOR_BLACK)
    self.render_boundary(
      self.lane_boundary_surface,
      self.lane_boundary,
      self.map_image.world_to_pixel,
    ...
    if render_types == None:
      ...
      if 'boundary' in render_types:
        surfaces.append((self.boundary_surface,(0,0)))
        surfaces.append((self.lane_boundary_surface,(0,0)))
